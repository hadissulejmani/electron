...
const isChromeExtension = function (pageURL) {
  ...
  return extensionProtocols.has(protocol)
}
...
ipcMainUtils.handleSync('CHROME_GET_MESSAGES', function (event, extensionId, language) {
  assertChromeExtension(event.sender, 'chrome.i18n.getMessage()')
  return getMessages(extensionId, language)
})
...
ipcMainUtils.handle('CHROME_STORAGE_READ', async function (event, storageType, extensionId) {
  assertChromeExtension(event.sender, 'chrome.storage.*.get()')
...
ipcMainUtils.handle('CHROME_STORAGE_WRITE', async function (event, storageType, extensionId, data) {
  assertChromeExtension(event.sender, 'chrome.storage.*.set()')
